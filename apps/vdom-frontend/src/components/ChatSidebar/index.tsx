...
import { StarredMessages } from "../StarredMessages/index";
...
interface ChatProps {
  ...
  chatActions: {
    ...
    starMessage: (message: any) => Promise<void>;
...
export const Chat = registerCustomElement(
  ...
  ({ isOpen = false, OnClose, chatState, chatActions }: ChatProps) => {
    const { messages, isLoading, error, starredMessages, isStarredMessagesOpen, isLoadingStarredMessages } = chatState;
    const { sendMessage, clearMessages, toggleStarredMessages, starMessage, unstarMessage, loadStarredMessages } = chatActions;
    ...
    const handleStarMessage = async (message: any) => {
      // Only allow starring user messages
      if (message.type !== 'user') return;
      
      try {
        if (message.isStarred) {
          // Find the starred message by content to unstar it
          const starredMessage = starredMessages.find(
            starred => starred.messageContent === message.content
          );
          if (starredMessage) {
            await unstarMessage(starredMessage._id);
          }
        } else {
    ...
    const handleStarredMessageClick = (messageContent: string) => {
    ...
    return (
      <>
        <div style="position: fixed; top: 0; right: 0; bottom: 0; width: 500px; background: white; box-shadow: -4px 0 24px rgba(0, 0, 0, 0.1); z-index: 2000; display: flex; flex-direction: column;">
          ...
          <div style="padding: 20px; border-bottom: 1px solid #e5e7eb; background: #f8fafc;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
              ...
              <div style="display: flex; gap: 8px;">
                <oj-button
                  onojAction={toggleStarredMessages}
          ...
          <div style="flex: 1; overflow-y: auto; padding: 20px;">
            {messages.map((message) => (
              <div
                key={message.id}
                style={`margin-bottom: 16px; display: flex; align-items: flex-start; gap: 8px; ${message.type === 'user' ? 'justify-content: flex-end;' : 'justify-content: flex-start;'}`}
              >
                {/* Star button for user messages */}
                {message.type === 'user' && !message.loading && (
                  <oj-button
                    onojAction={() => handleStarMessage(message)}
                    title={message.isStarred ? "Remove from starred" : "Add to starred"}
                    style="min-width: auto; padding: 4px 8px; font-size: 0.875rem; background: transparent; border-radius: 6px; color: #6b7280; margin-top: 4px;"
                  >
                    {message.isStarred ? '⭐' : '☆'}
                  </oj-button>
                )}
        ...
        <StarredMessages
          ...
          OnClose={toggleStarredMessages}
          starredMessages={starredMessages}
          isLoading={isLoadingStarredMessages}
          OnLoadStarredMessages={loadStarredMessages}
